...
public extension Date {
    ...
    static var allComponents: Set<Calendar.Component> = [.era, .year, .month, .day, .hour,
                                                         .minute, .second, .weekday, .weekdayOrdinal,
                                                         .quarter, .weekOfMonth, .weekOfYear,
...
extension DateComponents {
    ...
    public func description(
        ...
        if style == .relative {
            guard var string = formatter.string(from: self) else {
                return "\(self)"
            }
        ...
        if style == .approximate {
            ...
            var string = "About "
            if ti < Date.minuteInterval {
                string += "\(lrint(ti)) seconds"
            } else if abs(ti - Date.minuteInterval) <= 3.secondInterval {
                string += "a minute"
            } else if ti < Date.hourInterval {
                string += "\(lrint(ti / Date.minuteInterval)) minutes"
            } else if abs(ti - Date.hourInterval) <= (30.minuteInterval) {
                string += "an hour"
            } else if ti < Date.dayInterval {
                string += "\(lrint(ti / Date.hourInterval)) hours"
            } else if abs(ti - Date.dayInterval) <= (12.hourInterval) {
                string += "a day"
            } else if ti < Date.weekInterval {
                string += "\(lrint(ti / Date.dayInterval)) days"
            } else {
                string += "\(lrint(ti / (Date.weekInterval))) weeks"
            }
            if let newTime = Date.sharedCalendar.date(byAdding: self, to: Date()) {
                if newTime.isFuture {
                    string += " from now"
                } else if newTime.isPast {
                    string += " ago"
                }
            }
            return string
